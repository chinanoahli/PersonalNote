---
title: "Nintendo Switch SXOS 转 Atmosphère"
type: docs
draft: false
notHomePage: true
bookHidden: true
description: "Switch from SXOS to Atmosphère"
summary: "Nintendo Switch SXOS 转 Atmosphère"
---

# Nintendo Switch SXOS 转 Atmosphère

> 本教程不包含两个系统之间的虚拟系统互相转换教程，请转换前备份好各个游戏的存档
>
> SXOS 生成的虚拟系统无需保留，因为要将其转换成 Atmosphère 能用的并不容易，建议删掉后重新使用 **hekate - Nyx** 创建虚拟系统

1. 请先备份好所有你SXOS的启动文件以及许可证和启动文件，因为SXOS已经倒闭，其官网也无法再进入
   > [!IMPORTANT]
   >
   > 一旦遗失你的许可证，你就永远无法再启动SXOS了
   >
   > 因此你每次更新都必须做好备份
...
3. 下载 SX Gear：[下载地址 (需要翻墙)](https://web.archive.org/web/20210217231219/https://sx.xecuter.com/download/SX_Gear_v1.1.zip)
   ...
   >
   > [SX_Gear_v1.1.zip](https://attachments.chinanoahli.info/game/nintendo/switch/SX_Gear_v1.1.zip)
...
6. 下载 Signature Patches 签名补丁：
   ...
   > 2026 Jun 07 Update
   >
   > 上面的地址已经失效，如果要下载签名补丁，请前往[这里](https://wiidatabase.de/switch-downloads/hacks/signatur-patches/)
   >
...
8. 先备份原始的SXOS相关的文件
   ...
   + 将内存卡中的 boot.dat 复制到 `D:\SwitchCrack` 中并命名为 **boot_sxos.dat**

       重命名是为了防止以外删除，当然你也可以多备份到别的几个地方

   + 将内出卡中的 license.dat 复制到 `D:\SwitchCrack` 中

       **无论如何一定要备份好本文件，请备份到多个网盘和不同的地方**

9. 将前面下载到的文件解压到 `D:\SwitchCrack`

   > [!TIP] 按此顺序解压可以不用手动建立文件夹
   >
   > 1. SX_Gear_v1.1.zip
...
10. 将 fusee.bin 复制到 `D:\SwitchCrack\bootloader\payloads` 并重命名为 **atmosphere-X.X.X-fusee.bin**
    ...
    > X.X.X 为 Atmosphère 的版本号，自己根据实际修改
...
12. 新建文件 `hekate_ipl.ini` 并放入 `D:\SwitchCrack\bootloader` ，文件内容如下，注意修改 `[AutoSelect]` 处的 Atmosphère 版本号需要修改
...
15. 至此已经成功从 SXOS 切换到 Atmosphère，将 `D:\SwitchCrack\` 中的所有文件，覆盖到你 Switch 内存卡的根目录下，重新开机即可
...
17. \[非必要\]用Lockpick_RCM导出 `prod.keys` <br />(此工具已被任天堂举报，现已更名为 `Picklock_RCM` )
   ...
   >
   > 地址可能失效，老任经常举报，失效了就……自己想办法吧

   > 5. 下载 `Picklock_RCM.bin` 并更名为 `Lockpick_RCM.bin` (下载方法同第4步)
   > 6. 将该文件放到 `D:\SwitchCrack\bootloader\payloads`
   > 7. 开机进入hekate引导程序后，点击最上方的 `Console Info` 标签
   > 8. 点击 **SoC \& HW Info** 下方的 `Lockpick` 按钮，然后点击 `Continue` 按钮
   > 9. 进入Lockpick后，选择 `Dump from SysNAND`
   >    (用 `Vol +` 或 `Vol -` 键进行导航，然后按下 `Power` 确认选择)
   > 10. Lockpick会自动将 `prod.keys` 导出到 `sd:\switch\prod.keys`
   > 11. 按 `Power` 返回上一层，然后根据需要选择重启或关机即可

21. 几个自制的启动项目图标

    覆盖到 `D:\SwitchCrack\bootloader\res`，然后修改 `hekate_ipl.ini` 中的 `icon` 字段即可

    [res.7z](https://attachments.chinanoahli.info/game/nintendo/switch/res.7z)