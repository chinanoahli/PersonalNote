...
> 收录一些 macOS 系统目前遇到过的问题的解决方案。
>
> 方法是否过气请自行测试，我一开始用 Mac 是 2014 年左右，用了大概有 5 年左右，之后因为穷，一直没换代，就很少用
> 
> 直到 2026 年年初重新开始用 Mac，因此一些解决方案是 2010 年代的老方案，可能不适用于新版本 macOS

> `sudo` 要求输入管理员帐号的密码，输入密码时不会显示 **∙** 或 **\***，确保输入无误后按 **enter/return** 即可。

---

## 禁用 *「App」尚未针对您的Mac优化* 提示 [^1] [^2]
...
使用前请在 PKG 包上右键看看有没有 `显示包内容` 选项，如果有，证明这个安装包并非单文件，而是文件夹，直接点击 `显示包内容` 即可查看内容。

> PKG 指的是 PKG 格式的 macOS 软件包安装文件，并非所有 PKG 都能使用此技巧！
...
## 打开/关闭 *SIP*[^3] 安全机制(高危操作)

> 本操作可能导致系统不可逆损坏，请在执行前确保你清楚自己正在干什么！！

使用此方法需要先将Mac关机，然后按住 `Command + R` 开机，请一直按着 `Command + R` 直到完全进入「[恢复功能](https://support.apple.com/zh-cn/HT201314)」模式。
...
## 在「恢复模式[^4]」模式下修复全盘文件权限错误

> 此方法**仅**适用于**尚未**应用 *SIP* 安全机制的系统 (OS X El Capitan 之前的系统，不包括 OS X El Capitan) ，相关命令行工具在包含SIP的系统上已经被苹果**移除**！因此强烈**不**建议在日常使用中长时间关闭 SIP 保护！

> [!TIP] Apple silicon
>
> 在 **关机** 状态下，长按 **电源键** 10 秒

> [!TIP] Intel
>
> 在 **关机** 状态下，然后按住 `Command + R` 开机，请一直按着 `Command + R` 直到完全进入「恢复模式」模式。

在「恢复模式」模式下打开 *Terminal* 根据需要运行以下命令，假设你的系统盘盘符为「Macintosh HD」
...
macOS 系统中只能同时指定一个 XCode 作为开发工具，如果有多个不同版本的 XCode 需要手动切换。
...
> 在 HFS+ 文件系统里，本操作可能使软连接失效，使用时请注意。

macOS 的 Finder 可以给文件或文件夹加上颜色标签等扩展属性，此功能需要创建一个名为`._原文件名`的文件，在 macOS 上表现为通过终端列出此文件会带有特殊的「@」属性。
...
> 本操作会重置 Launchpad ，所有系统应用的图标都会回到系统**默认**的位置，而自己安装的应用则会**随机**排列到第二页及以后的位置。

本操作可以解决 Launchpad 里面还残留了已经删除的 App 图标，或者出现图标错位，删除不了的情况。
...
本操作利用 macOS 的「[@](#去除文件特殊属性)」属性隐藏，而非 Unix 标准的将文件名或文件夹名改成 `.` 开头的办法。
...
> [!TIP] Intel Only
>
> 启用了 *SIP* 的系统，若要修改 `/Applications` 中的系统应用，请到「恢复模式」模式下使用本命令；或先行关闭 SIP ，修改完毕后再重新打开 SIP

> [!WARNING] Apple silicon
>
> 在 Apple silicon 平台，请不要尝试修改系统自带的应用，没错，就算是加隐藏属性也不行
>
> 在 Apple silicon 平台中，主板固件、系统内核以及内建应用均有签名验证机制，只要签名验证不通过，就一定会启动失败
...
> 本操作可以禁止任意分区（可以是任意文件系统，如：NTFS、HFS+等）在系统开机、重启、或拔出 USB 硬盘后重新连接时自动挂载
...
> **Volume UUID** 是在分区格式化时由系统自动生成的，此后直到下一次格式化为止一般不再改变
> 
...
[^3]: [System Integrity Protection](https://support.apple.com/zh-cn/HT204899)
[^4]: [macOS Recovery](https://support.apple.com/zh-cn/HT201314)
[^5]: [如何搜索适用于您的文件的应用](https://support.apple.com/zh-cn/HT201290)
